...
function getFilledRectCells(a: CellInfo, b: CellInfo): CellInfo[] {
  ...
  const cells: CellInfo[] = [];
  for (let cx = minX; cx <= maxX; cx++)
    for (let cz = minZ; cz <= maxZ; cz++) cells.push(cell(cx, cz));
  return cells;
}
...
function getFloodFillCells(
  ...
  const outputs = game.dungeon.outputs as {
    rooms?: Map<
      number,
...
export default function DungeonView() {
  ...
  useEffect(() => {
    ...
    if (imported) {
      ...
      // restore editable textures that g.generate() overwrites from the seed
      function copyTex(
        dst: THREE.DataTexture | undefined,
        srcData: Uint8Array | undefined,
      ) {
        if (dst && srcData) {
          (dst.image.data as Uint8Array).set(srcData);
          dst.needsUpdate = true;
        }
      }
      copyTex(outputs.textures.regionId, src.regionId.image.data as Uint8Array);
      copyTex(
        outputs.textures.colliderFlags,
        src.colliderFlags.image.data as Uint8Array,
      );
      copyTex(
        outputs.textures.floorSkirtType,
        src.floorSkirtType.image.data as Uint8Array,
      );
      copyTex(
        outputs.textures.ceilSkirtType,
        src.ceilSkirtType.image.data as Uint8Array,
      );

      // keep fullRegionIds in sync with the restored regionId data
      const bspOutputs = outputs as {
        rooms?: Map<number, { type: string }>;
        fullRegionIds?: Uint8Array;
      };
      if (bspOutputs.fullRegionIds) {
        bspOutputs.fullRegionIds.set(src.regionId.image.data as Uint8Array);
      }
      ...
      const floorSrc = src.floorHeightOffset?.image.data as
        | Uint8Array
        | undefined;
      const ceilSrc = src.ceilingHeightOffset?.image.data as
        | Uint8Array
        | undefined;
      copyTex(floorDst, floorSrc);
      copyTex(ceilDst, ceilSrc);
      ...
      for (let cz = 0; cz < H; cz++) {
        for (let cx = 0; cx < W; cx++) {
          ...
          if (f !== 128 || c !== 128)
            heights[`${cx},${cz}`] = { floor: f, ceil: c };
        }
      }
      setCellHeights(heights);

      // restore room types
      if (imported.dungeon.rooms.size > 0) {
        for (const [id, info] of imported.dungeon.rooms) {
          const room = bspOutputs.rooms?.get(id);
          if (room) room.type = info.type;
        }
      }
      ...
      const paints: Record<
        string,
        import("../DataContext").SurfacePaintTarget
      > = {};
    ...
    renderer = createDungeonRenderer(el, game, {
      ...
      onCellClick(info) {
        ...
        if (tool === "rect") cells = getRectBorderCells(pending, info);
          ...
          cells = getFilledRectCells(pending, info);
